...
- 先用本地目录跑起来。
- 再看怎么同时配置 local、S3、OSS，并用参数指定这次文件要写到哪个 disk。
...
你可以从命令行、环境变量、租户配置或后台表单拿到 disk 名称：
...
err = s3Disk.Put(ctx, "reports/hello.txt", []byte("hello"))
```
...
err = ossDisk.Put(ctx, "reports/hello.txt", []byte("hello"))
```
