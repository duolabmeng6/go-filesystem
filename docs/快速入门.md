...
```go
if err := manager.MustDisk("local").Put(ctx, "reports/local.txt", data); err != nil {
	return err
}

if err := manager.MustDisk("s3").Put(ctx, "reports/s3.txt", data); err != nil {
	return err
}

if err := manager.MustDisk("oss").Put(ctx, "reports/oss.txt", data); err != nil {
	return err
}
```

如果要从命令行切换，也只是把 disk 名称换成环境变量。disk 名称来自外部输入时，用 `Disk` 接住错误：
...
```go
if err := manager.MustDisk("s3").Put(ctx, "reports/hello.txt", []byte("hello")); err != nil {
	return err
}
```
...
```go
if err := manager.MustDisk("oss").Put(ctx, "reports/hello.txt", []byte("hello")); err != nil {
	return err
}
```
