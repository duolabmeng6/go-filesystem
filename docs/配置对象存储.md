...
推荐用法：一次性配置 `local`、`s3`、`oss` 等多个 disk。普通业务走默认 disk；需要动态切换时，用 `manager.Disk(name)` 指定这次文件写到哪个驱动。
...
err = disk.Put(ctx, "uploads/a.txt", []byte("hello"))
```
...
if err := s3Disk.Put(ctx, "uploads/a.txt", []byte("hello")); err != nil {
	return err
}
