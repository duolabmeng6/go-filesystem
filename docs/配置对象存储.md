...
```go
if err := manager.MustDisk("local").Put(ctx, "uploads/local.txt", data); err != nil {
	return err
}

if err := manager.MustDisk("s3").Put(ctx, "uploads/s3.txt", data); err != nil {
	return err
}

if err := manager.MustDisk("oss").Put(ctx, "uploads/oss.txt", data); err != nil {
	return err
}
```
...
```go
if err := manager.MustDisk("s3").Put(ctx, "uploads/a.txt", []byte("hello")); err != nil {
	return err
}

data, err := manager.MustDisk("s3").Get(ctx, "uploads/a.txt")
url, err := manager.MustDisk("s3").URL(ctx, "uploads/a.txt")
temporaryURL, err := manager.MustDisk("s3").TemporaryURL(ctx, "uploads/a.txt", time.Now().Add(15*time.Minute))
```

如果 disk 名称来自命令行、环境变量、租户配置或后台表单，请用 `Disk` 处理错误：
...
```go
url, err := manager.MustDisk("oss").TemporaryURL(
	ctx,
	"images/a.jpg",
